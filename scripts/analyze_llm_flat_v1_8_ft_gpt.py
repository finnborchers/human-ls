...
OUT_PATH = os.getenv(
    ...
    "analysis/llm/benchmark_comparison/reviews_v1_8_holdout20_labeled.json",
)
JOB_PATH = Path(
    os.getenv("FINETUNE_JOB_OUT_PATH", "analysis/llm/finetune/reviews_v1_8_finetune_job.json")
)
MODEL = os.getenv("REVIEW_LABELS_MODEL")
...
def resolve_model_name() -> str:
    ...
    if not JOB_PATH.exists():
        raise FileNotFoundError(
            f"No REVIEW_LABELS_MODEL set and no fine-tuning job file found: {JOB_PATH}"
        )

    with JOB_PATH.open("r", encoding="utf-8") as f:
        job_state = json.load(f)

    fine_tuned_model = job_state.get("fine_tuned_model")
    if not fine_tuned_model:
        raise ValueError(f"Fine-tuning job file does not contain a fine_tuned_model yet: {JOB_PATH}")
